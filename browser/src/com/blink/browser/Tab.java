...
public class Tab implements PictureListener {
    ...
    private final WebViewClient mWebViewClient = new WebViewClient() {
        ...
        public void onPageStarted(WebView view, String url, Bitmap favicon) {
            ...
            //AdBlock.getInstance().onPageStarted(url);
            mInPageLoad = true;
        ...
        public void onPageFinished(WebView view, String url) {
            ...
            //AdBlock.getInstance().onPageFinished(view, url);
        }
...
/*        @Override
        ...
        public WebResourceResponse shouldInterceptRequest(WebView view,
            ...
            WebResourceResponse response =
                    null;//AdBlock.getInstance().shouldInterceptRequest(mContext, url);
            if (response != null) {
                if (isCount && !TextUtils.isEmpty(url)) {
                    if (UrlUtils.checkUrlDataIsImg(url)) {
                        mImgAdCount++;
                    } else if (UrlUtils.checkUrlDataIsJs(url)) {
                        mJsAdCount++;
                    } else {
                        mPopupAdCount++;
                    }
...
/*        @Override
    ...
    private class SubWindowChromeClient extends WebChromeClient {
        ...
        @Override
        public void onCloseWindow(WebView window) {
            if (mSubView != null && window != mSubView) {
        ...
        mWebBackForwardListClient = new WebBackForwardListClient() {
            ...
            public void onNewHistoryItem(WebHistoryItem item) {
                if (mClearHistoryUrlPattern != null) {
                    ...
                    Logger.debug(LOGTAG, "onNewHistoryItem: match=" + match + "\n\t"
                            + item.getUrl() + "\n\t"
     ...
     * the container view.
     */
    void setWebView(WebView w, final boolean restore) {
        if (mMainView == w) {
            return;
        }
        ...
        // set the new one
        mMainView = w;
        if (mMainView  instanceof BrowserWebView && null != mCurrentState) {
        ...
        if (mMainView != null) {
            ...
            if (tc != null && tc.getOnThumbnailUpdatedListener() != null) {
            	mMainView.setPictureListener(Tab.this);
            }
    ...
    void resume() {
        if (mMainView != null) {
            ...
            if (mInForeground) {
                ImmersiveController.getInstance().setWebViewStatusBarColor(ContextCompat.getColor(mContext, R.color.status_bar_webview));
                setWebViewThemeColor(mMainView);
            }
        }
    }
    ...
    public String getUrl() {
        if (mIsNativePage) {
            return BrowserContract.NATIVE_PAGE_URL;
        }
    ...
    private class SaveCallback implements ValueCallback<String> {
        ...
        public void onReceiveValue(String path) {
            ...
            BrowserAnalytics.trackEvent(BrowserAnalytics.Event.MENU_EVENTS, AnalyticsSettings.ID_SAVEPAGE, value);
...
//        values.put(Snapshots.THUMBNAIL, compressBitmap(screenshot));
    ...
    public void saveViewState() {
        ...
        try {
            synchronized (callback) {
                web.saveWebArchive(finalPath, false, callback);
            }
        } catch (Exception e) {
            Logger.error(LOGTAG, "Failed to save view state", e);
    ...
    public void goBack() {
        if (getWebView() != null) {
            ...
            getWebView().goBack();
            setWebViewThemeColor(getWebView());
        }
    }
    ...
    void updateCaptureFromBitmap(Bitmap bitmap) {
        synchronized (this) {
    ...
    public void setAcceptThirdPartyCookies(boolean accept) {
        ...
        if (mMainView != null && Build.VERSION.SDK_INT >= BuildUtil.VERSION_CODES.LOLLIPOP) {
        	 try {
                 ...
                 Logger.error(LOGTAG, "the cookiemananger can't setAcceptThirdPartyCookies:" + e.getStackTrace());
             }
        }
    }
    ...
    public void setNativePage(boolean isNativeTab) {
        if (mIsNativePage && !isNativeTab) {//Status changed, add webview
            if (null == mMainView) {
                WebView newWebView = mWebViewController.getTabControl().createNewWebView();
                mCurrentState = new PageState(mContext, newWebView.isPrivateBrowsingEnabled());
                newWebView.setOnCreateContextMenuListener(mWebViewController.getActivity());
    ...
    public void setWebViewThemeColor(WebView view) {
        if (Build.VERSION.SDK_INT >= BuildUtil.VERSION_CODES.KITKAT) {
            try {
                view.evaluateJavascript(BrowserContract.JAVASCRIPT_THEME_COLOR, new ValueCallback<String>() {
                    @Override
        ...
        if (!TextUtils.isEmpty(color)) {
            ...
            if (!TextUtils.isEmpty(color)) {
                try {
                    ...
                    Logger.error(LOGTAG, "color is " + color);
                }
            }
        }
    }

    public void setSystemBarColorFromIcon(Bitmap icon) {
        if (mInForeground && !mSetThemeColorSuccess && icon != null) {
            Palette.from(icon).generate(new Palette.PaletteAsyncListener() {
                ...
                public void onGenerated(Palette palette) {
                    ...
                    if (swatch != null) {
                        if (mInForeground && !mSetThemeColorSuccess) {
                            ImmersiveController.getInstance().setWebViewStatusBarColor(swatch.getRgb());
                        }
                    }
                }
            });
