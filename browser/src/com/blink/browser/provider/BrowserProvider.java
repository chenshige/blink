...
public class BrowserProvider extends ContentProvider {
                ...
                switch(columnIndex) {
                    ...
                    case SUGGEST_COLUMN_ICON_1_ID:
                        if (type == 1) {
                            ...
                            return Integer.valueOf(
                                    R.drawable.ic_browser_home_swipe_left_search)
                                    .toString();
                        }
        ...
        switch (match) {
            case URI_MATCH_BOOKMARKS: {
                ...
                long rowID = db.insert(TABLE_NAMES[URI_MATCH_BOOKMARKS], "url",
                        initialValues);
                if (rowID > 0) {
                    uri = ContentUris.withAppendedId(BrowserHelper.BOOKMARKS_URI,
                            rowID);
                }
            ...
            case URI_MATCH_SEARCHES: {
                ...
                long rowID = db.insert(TABLE_NAMES[URI_MATCH_SEARCHES], "url",
                        initialValues);
                if (rowID > 0) {
                    uri = ContentUris.withAppendedId(BrowserHelper.SEARCHES_URI,
                            rowID);
                }
                break;
            }
    ...
    public int delete(Uri url, String where, String[] whereArgs) {
        ...
        if (isBookmarkTable) {
            Cursor cursor = cr.query(BrowserHelper.BOOKMARKS_URI,
