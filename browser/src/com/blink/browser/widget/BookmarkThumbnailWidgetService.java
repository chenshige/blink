...
import com.blink.browser.reflection.ContextExtension;
import com.blink.browser.reflection.RemoteViewsExtension;
...
public class BookmarkThumbnailWidgetService extends RemoteViewsService {
    ...
    static void deleteWidgetState(Context context, int widgetId) {
        File file = ContextExtension.getSharedPrefsFile(context,
                String.format("widgetState-%d", widgetId));
        if (file.exists()) {
            if (!file.delete()) {
                file.deleteOnExit();
    ...
    static void removeOrphanedStates(Context context, int[] widgetIds) {
        File prefsDirectory = ContextExtension.getSharedPrefsFile(context, "null").getParentFile();
    ...
    static class BookmarkFactory implements RemoteViewsService.RemoteViewsFactory {
        ...
        public RemoteViews getViewAt(int position) {
            ...
            if (isFolder) {
                ...
                RemoteViewsExtension.setDrawableParameters(views, R.id.thumb, true, 0, -1, null, -1);
            } else {
                ...
                RemoteViewsExtension.setDrawableParameters(views, R.id.thumb, true, 255, -1, null, -1);
            ...
            if (isFolder) {
                ...
                if (!TextUtils.isEmpty(url)) {
                    fillin = new Intent(BrowserActivity.ACTION_OPEN_URL, Uri.parse(url), mContext,
