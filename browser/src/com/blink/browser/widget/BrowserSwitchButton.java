...
public class BrowserSwitchButton extends CompoundButton {
    ...
    public BrowserSwitchButton(Context context, AttributeSet attrs, int defStyle) {
        super(context, attrs, defStyle);
        init(attrs);
    }

    public BrowserSwitchButton(Context context, AttributeSet attrs) {
        this(context, attrs, 0);
    }

    public BrowserSwitchButton(Context context) {
        this(context, null);
    }
    ...
    private void init(AttributeSet attrs) {
        ...
        if (ta != null) {
            thumbDrawable = ta.getDrawable(R.styleable.SwitchButton_browserThumbDrawable);
            thumbColor = ta.getColorStateList(R.styleable.SwitchButton_browserThumbColor);
            margin = ta.getDimension(R.styleable.SwitchButton_browserThumbMargin, margin);
            marginLeft = ta.getDimension(R.styleable.SwitchButton_browserThumbMarginLeft, margin);
            marginRight = ta.getDimension(R.styleable.SwitchButton_browserThumbMarginRight, margin);
            marginTop = ta.getDimension(R.styleable.SwitchButton_browserThumbMarginTop, margin);
            marginBottom = ta.getDimension(R.styleable.SwitchButton_browserThumbMarginBottom, margin);
            thumbWidth = ta.getDimension(R.styleable.SwitchButton_browserThumbWidth, thumbWidth);
            thumbHeight = ta.getDimension(R.styleable.SwitchButton_browserThumbHeight, thumbHeight);
            thumbRadius = ta.getDimension(R.styleable.SwitchButton_browserThumbRadius, Math.min(thumbWidth, thumbHeight) / 2.f);
            backRadius = ta.getDimension(R.styleable.SwitchButton_browserBackRadius, thumbRadius + density * 2f);
            backDrawable = ta.getDrawable(R.styleable.SwitchButton_browserBackDrawable);
            backColor = ta.getColorStateList(R.styleable.SwitchButton_browserBackColor);
            backMeasureRatio = ta.getFloat(R.styleable.SwitchButton_browserBackMeasureRatio, backMeasureRatio);
            animationDuration = ta.getInteger(R.styleable.SwitchButton_browserAnimationDuration, animationDuration);
            fadeBack = ta.getBoolean(R.styleable.SwitchButton_browserFadeBack, true);
            tintColor = ta.getColor(R.styleable.SwitchButton_browserTintColor, tintColor);
            textOn = ta.getString(R.styleable.SwitchButton_browserTextOn);
            textOff = ta.getString(R.styleable.SwitchButton_browserTextOff);
            textMarginH = ta.getDimension(R.styleable.SwitchButton_browserTextMarginH, textMarginH);
            ta.recycle();
        }
    ...
    private void setup() {
        ...
        if (mBackDrawable != null) {
            mBackDrawable.setBounds((int) mThumbRectF.left, (int) (mBackRectF.top + mBackRectF.height() * 1 / 5), (int) (mBackRectF.right - mThumbMargin.right), (int) (mBackRectF.bottom - (int) mBackRectF.height() * 1 / 5));
        }
    ...
    protected void onDraw(Canvas canvas) {
        ...
        if (mIsBackUseDrawable) {
            ...
            if (mFadeBack) {
                ...
                canvas.drawRoundRect(mBackRectF, -200, mBackRadius, mPaint);
    ...
    protected void drawableStateChanged() {
        ...
        int[] nextState = isChecked() ? CHECKED_PRESSED_STATE : UNCHECKED_PRESSED_STATE;
    ...
    protected void animateToState(boolean checked) {
        ...
        mProcessAnimator.setDuration(mAnimationDuration);
        if (isLayoutRtl()) {
            mProcessAnimator.setFloatValues(mProcess, checked ? 0 : 1f);
        } else {
            mProcessAnimator.setFloatValues(mProcess, checked ? 1f : 0);
        }
    ...
    public boolean isLayoutRtl() {
        ...
        if (context != null && context instanceof Activity) {
            return ((Activity) context).getWindow().getDecorView().getLayoutDirection() == View.LAYOUT_DIRECTION_RTL;
        } else {
            return getParent().getLayoutDirection() == View.LAYOUT_DIRECTION_RTL;
        }
    }
