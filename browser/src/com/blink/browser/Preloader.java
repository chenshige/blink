...
public class Preloader {
    ...
    private PreloaderSession getSession(String id) {
        if (mSession == null) {
            Logger.debug(LOGTAG, "Create new preload session " + id);
            mSession = new PreloaderSession(id);
            WebViewTimersControl.getInstance().onPrerenderStart(
                    mSession.getWebView());
            return mSession;
        } else if (mSession.mId.equals(id)) {
            Logger.debug(LOGTAG, "Returning existing preload session " + id);
            return mSession;
        }

        Logger.debug(LOGTAG, "Existing session in progress : " + mSession.mId +
                " returning null.");
        return null;
    }
    ...
    public void handlePreloadRequest(String id, String url, Map<String, String> headers,
        ...
        if (s == null) {
            Logger.debug(LOGTAG, "Discarding preload request, existing"
                    + " session in progress");
            return;
        }
    ...
    public PreloadedTabControl getPreloadedTab(String id) {
        PreloaderSession s = takeSession(id);
        Logger.debug(LOGTAG, "Showing preload session " + id + "=" + s);
    ...
    private class PreloaderSession {
        ...
        private final Runnable mTimeoutTask = new Runnable(){
            @Override
            public void run() {
                Logger.debug(LOGTAG, "Preload session timeout " + mId);
                discardPreload(mId);
            }};
